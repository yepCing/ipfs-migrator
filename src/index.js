...
const DESTINATION_ENDPOINT = `https://api.4everland.dev`;
...
document.addEventListener("DOMContentLoaded", function (event) {
  document
    .getElementById("sourceSubmitButton")
    .addEventListener("click", async function (event) {
      ...
      try {
        ...
        alert(
          `An error occurred.  Please check your endpoint and credentials before trying again`
        );
      }
    });

  document
    .getElementById("destinationSubmitButton")
    .addEventListener("click", async function (event) {
      event.preventDefault();
      try {
  ...
  document
    .querySelector("#sync-btn-bg")
    .addEventListener("click", async function (event) {
      // Stop the form from submitting by default
      // event.preventDefault();
      await syncProviders();
      await loadDestination();
      document.querySelector("#sync-btn-bg").classList.remove("btn-syncing");
    });
});
...
async function loadDestination() {
  ...
  syncConfig.destination.endpointUrl =
    destinationLoginForm.elements.endpoint.value;
  syncConfig.destination.accessToken =
    destinationLoginForm.elements.token.value;
...
async function syncProviders() {
  try {
    // Start Sync Client
    const ipfsSyncClient = new IpfsPinSync(
      syncConfig.source,
      syncConfig.destination
    );
    document.querySelector("#sync-btn-bg").classList.add("btn-syncing");
