...
export default class IpfsPinSync {
  constructor(sourceConfigOptions, destinationConfigOptions) {
    if (
      typeof sourceConfigOptions !== "undefined" &&
      sourceConfigOptions !== {}
    ) {
    ...
    if (
      typeof destinationConfigOptions !== "undefined" &&
  ...
  listSource() {
    // if (this.sourceConfig.endpointUrl.indexOf("api.pinata.cloud") !== -1) {
  ...
  listDestination() {
    // if (this.destinationConfig.endpointUrl.indexOf("api.pinata.cloud") !== -1) {
    ...
    for (let sourcePin of sourcePins) {
      if (
        ...
        if (pinKeys.has(sourcePin.pin.name) === true) {
          duplicateKeys[sourcePin.pin.name] =
            duplicateKeys[sourcePin.pin.name] || 0;
          duplicateKeys[sourcePin.pin.name] =
            duplicateKeys[sourcePin.pin.name] + 1;
          sourcePin.pin.name = `${sourcePin.pin.name} [${
            duplicateKeys[sourcePin.pin.name]
          }]`;
        }
      ...
      const addPinRequest = throttledAddPin(
